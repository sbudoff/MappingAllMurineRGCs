...
#=============================================================================
  ...
  OUTPUT_DIR <- "GlobalStatistics_Smoothed"                 # Main output directory name
...
create_mask_features <- function(masks, grid) {
  ...
  mask_names <- c("ipsi", "binocular", "peripheral", "visual_sky", "visual_ground")
...
project_masks_to_umap <- function(mask_features, umap_model) {
  # Ensure columns match embedding_data
  required_cols <- names(embedding_data)
  feature_matrix <- mask_features[, required_cols]
...
# Main executionhttp://127.0.0.1:40221/graphics/plot_zoom_png?width=1992&height=1101
...
# Load functional data and prepare for merging
unification <- read_csv(paste0(ROOT_DIR, 'RGC Subtype Unification.csv')) %>%
  rename(cluster = Tran2019_Clusters,
         Spot_Size = `Spot_Size_(um)`) %>%
  select(cluster, Tran2019_Names, Goetz2022_PatchSeq, Goetz2022_Name, Functional_Group, 
...
# Define the numerical variables we'll analyze
...
# First modify cluster_assignments to rename Cluster column
cluster_assignments <- cluster_assignments %>%
  rename(spatial_cluster = Cluster)

# Extract numeric part from T[d] label and merge with unification
unified_functional_spatial <- cluster_assignments %>%
  mutate(cluster = as.numeric(str_extract(Label, "\\d+"))) %>%
  right_join(unification, by = "cluster") %>%
  select(spatial_cluster, Label, cluster, everything()) %>%
  mutate(spatial_cluster = factor(spatial_cluster))

# Function to run tests and create visualizations
run_kw_tests <- function(data, var) {
  # Create formula for test
  formula_str <- paste(var, "~ spatial_cluster")
  
  # Run Kruskal-Wallis
  kw_result <- stats::kruskal.test(formula(formula_str), data = data)
  
  # Create boxplot
  p <- ggplot2::ggplot(data, ggplot2::aes(x = factor(spatial_cluster), y = !!sym(var))) +
    ggplot2::geom_boxplot(width = 0.7) +
    ggplot2::geom_jitter(width = 0.2, alpha = 0.5) +
    ggplot2::theme_classic() +
    ggplot2::labs(title = var,
                  subtitle = sprintf("Kruskal-Wallis raw p = %.3f", kw_result$p.value),
                  x = "Spatial Cluster",
                  y = var)
  ...
  return(list(
    ...
    plot = p,
    n_per_group = table(data$spatial_cluster)
  ))
}

# Run analysis for each variable
kw_results <- lapply(numerical_vars, function(var) {
  run_kw_tests(unified_functional_spatial, var)
})

# Apply Bonferroni correction to all KW p-values
bonf_corrected_pvals <- stats::p.adjust(sapply(kw_results, function(x) x$kw_pvalue), 
                                        method = "bonferroni")

# Run Dunn's test only for significant corrected results
for(i in seq_along(kw_results)) {
  if(bonf_corrected_pvals[i] < 0.05) {
    formula_str <- paste(kw_results[[i]]$variable, "~ spatial_cluster")
    dunn_result <- FSA::dunnTest(formula(formula_str), 
                                 data = unified_functional_spatial,
                                 method = "bh")
    kw_results[[i]]$significant <- TRUE
    kw_results[[i]]$dunn <- dunn_result$res
  } else {
    kw_results[[i]]$significant <- FALSE
  }
  kw_results[[i]]$corrected_pvalue <- bonf_corrected_pvals[i]
}
...
for(i in seq_along(kw_results)) {
  ...
  cat("Kruskal-Wallis raw p-value:", format.pval(res$kw_pvalue, digits = 3), "\n")
  cat("Bonferroni corrected p-value:", format.pval(res$corrected_pvalue, digits = 3), "\n")
  cat("Sample sizes per group:\n")
  print(res$n_per_group)
...
# Update plot subtitles with corrected p-values
for(i in seq_along(kw_results)) {
  kw_results[[i]]$plot <- kw_results[[i]]$plot + 
    ggplot2::labs(subtitle = sprintf("KW p = %.3f (corrected p = %.3f)", 
                                     kw_results[[i]]$kw_pvalue,
                                     kw_results[[i]]$corrected_pvalue))
}

# Combine all plots
plot_grid <- ggpubr::ggarrange(plotlist = lapply(kw_results, function(x) x$plot), 
                               ncol = 2, 
                               nrow = ceiling(length(numerical_vars)/2))

# Display plot grid
print(plot_grid)

# Save plot
ggplot2::ggsave(file.path(ROOT_DIR, "functional_analysis_results.pdf"), 
                plot_grid, 
                width = 12, 
                height = 2 * ceiling(length(numerical_vars)/2))


#' @param grid Reference grid matrix
#' @param mask Target mask to be plotted
#' @return List containing mask matrix and visualization plot
plot_retinal_mask <- function(grid, mask){
...
for (i in 1:length(masks)) {
 plot_retinal_mask(grid, masks[[i]])
}
...
###################################################################################3
analyze_mask_cluster_overlap <- function(masks, grid, scan_results, ROOT_DIR, f1_results) {
  # Create output directory
  output_dir <- file.path(ROOT_DIR, "masked_regions_overlap")
  ...
  for(i in seq_along(scan_results)) {
    ...
    # Skip if no clusters
    if(is.null(current_result$clusters)) {
      message(sprintf("No significant clusters found for layer %d", i))
      next
    }
    ...
    # Get original data and normalize it
    original_data <- tif_stack[[i]]
    original_data[original_data <= 0] <- NA
    
    # Get cluster boundaries for plotting
    boundary_points <- data.frame()
    sig_clusters <- smerc::clusters(current_result)
    if(!is.null(sig_clusters)) {
      for(j in seq_along(sig_clusters)) {
        cluster_cells <- sig_clusters[[j]]
        boundary <- find_cluster_boundaries(grid, cluster_cells)
        if(any(boundary)) {
          boundary_points <- rbind(
            boundary_points,
    ...
    p1 <- ggplot(
      data = expand.grid(row = 1:nrow(grid), col = 1:ncol(grid)) %>%
        mutate(value = as.vector(original_data)),
      aes(x = col, y = row)
    ) +
      geom_tile(aes(fill = value)) +
      scale_fill_viridis_c(na.value = "white") +
      geom_point(data = boundary_points, color = "green3", size = 0.5) +
      coord_equal() +
      theme_void() +
      theme(legend.position = "none")
    ...
    p2 <- ggplot(
      data = expand.grid(row = 1:nrow(grid), col = 1:ncol(grid)) %>%
        mutate(value = as.vector(original_data * cluster_mask)),
      aes(x = col, y = row)
    ) +
      geom_tile(aes(fill = value)) +
      scale_fill_viridis_c(na.value = "white") +
    ...
    # Add the original plot pair with title
    plots[[1]] <- wrap_plots(
      ggplot() + 
        annotate("text", x = 0.5, y = 0.5, 
                 label =sprintf("Layer %d - Cluster and Mask Analysis", i), size=10) +
        theme_void() +
    ...
    for(mask_name in names(masks)) {
      ...
      # Get F1 score
      f1_col <- paste0(mask_name, "_F1")
      f1_score <- f1_results$results[[f1_col]][i]
      f1_percent <- round(f1_score * 100, 1)
      
      # Create masked plots
      masked_data <- original_data * mask
      overlap_data <- original_data * cluster_mask * mask
      
      p1 <- ggplot(
        data = expand.grid(row = 1:nrow(grid), col = 1:ncol(grid)) %>%
          mutate(value = as.vector(masked_data)),
        aes(x = col, y = row)
      ) +
        geom_tile(aes(fill = value)) +
        scale_fill_viridis_c(na.value = "white") +
      ...
      p2 <- ggplot(
        data = expand.grid(row = 1:nrow(grid), col = 1:ncol(grid)) %>%
          mutate(value = as.vector(overlap_data)),
        aes(x = col, y = row)
      ) +
        geom_tile(aes(fill = value)) +
        scale_fill_viridis_c(na.value = "white") +
      ...
      # Add the plot pair with title and F1 score
      plots[[plot_counter]] <- wrap_plots(
        ggplot() + 
          annotate("text", x = 0.5, y = 0.5, 
                   label = sprintf("%s F1=%.1f%%", mask_name, f1_percent), size=10) +
          theme_void() +
          theme(plot.margin = margin(0, 0, 0, 0)),
        wrap_plots(p1, p2, ncol = 2),
        ncol = 1,
    ...
    # Arrange in a grid with 5 rows
    n_plots <- length(plots)
    n_cols <- ceiling(n_plots / 5)
    
    # Add empty plots if needed to fill the grid
    while(length(plots) < n_cols * 5) {
    ...
    ggsave(
      filename = file.path(output_dir, sprintf("layer_%d_mask_overlap.png", i)),
      plot = combined_plot,
      width = min(8 * n_cols, 40),
      height = 40,
      dpi = 300,
      limitsize = FALSE
    )
...
analyze_mask_cluster_overlap(masks, grid, scan_results$results, main_dir, f1_results)
