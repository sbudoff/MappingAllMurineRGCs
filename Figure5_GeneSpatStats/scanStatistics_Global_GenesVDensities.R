...
#=============================================================================
  ...
  #=============================================================================
  ROOT_DIR <- "/home/sam/FinalRGC_xenium/Gene_Map_Analysis_VsDensities/"               # Base directory for analysis
  TIF_PATH <- "M_RGC_Genes_Plot.tif"        # Input TIF file containing cell type maps
  OUTPUT_DIR <- "GlobalStatistics_Subtypes"                 # Main output directory name
  ...
  N_SIM <- 500           # Number of Monte Carlo simulations for scan statistics
  ...
  # Bootstrap Analysis
  N_BOOTSTRAP <- 100    # Number of bootstrap iterations for visual scene analysis
...
# Moran's I Analysis
#=============================================================================
create_morans_fallback_files <- function(tif_stack, output_dir) {
  # Create fallback results dataframe with NA values
  fallback_results <- data.frame(
    Layer = seq_along(tif_stack),
    MoransI = NA_real_,
    Expected = NA_real_,
    PValue = NA_real_,
  ...
  # Create empty significant layers dataframe
  fallback_significant <- data.frame(
    SignificantLayers = integer(0),
    stringsAsFactors = FALSE
  )
  
  # Save fallback results
  write.csv(fallback_results, 
            file.path(output_dir, "morans_results.csv"), 
            row.names = FALSE)
  
  # Save empty significant layers file
  write.csv(fallback_significant,
...
if (GENE_MAP_ANALYSIS) {
  ...
  rgc_path <- "/home/sam/FinalRGC_xenium/rgc_expMat_with_Studyregions_transformedCoords.csv"
...
if (!file.exists( stats_path)) {
  ...
  tryCatch(
    ...
    error = function(e) {
      message("Error in Moran's analysis: ", e$message)
      morans_results <- create_morans_fallback_files(tif_stack, dirs$morans)
      clustered_rgc_genes <- valid_rgc_genes 
      return(NULL)
    }
  )
...
update_comprehensive_statistics <- function(comp_stats_path, 
  ...
  comp_stats_igor <- comp_stats %>%
    mutate(across(everything(), ~replace(., is.na(.), -1000)))
  
  write.csv(comp_stats_igor, paste0(substr(comp_stats_path, 1,nchar(comp_stats_path)-4), "_igor.csv"), row.names = FALSE)
...
update_comprehensive_statistics <- function(comp_stats_path,
                                            density_f1_results,
                                            density_enrichment_results,
                                            valid_rgc_genes = NULL,
                                            is_gene_analysis = FALSE) {
  
  # Read existing comprehensive statistics
  comp_stats <- read.csv(comp_stats_path)
  
  # If gene analysis, prepare the mapping dataframe
  if(is_gene_analysis) {
    if(is.null(valid_rgc_genes)) {
      stop("valid_rgc_genes must be provided when is_gene_analysis is TRUE")
    }
    
    # Prepare valid_rgc_genes with consistent column names
    gene_mapping <- valid_rgc_genes %>%
      rename(subtype = Prediction, gene = column) %>%
      mutate(original_index = row_number())
  }
  
  # Extract and process F1 scores
  f1_scores <- density_f1_results$results
  
  if(is_gene_analysis) {
    # Add gene/subtype information to F1 scores
    f1_scores <- f1_scores %>%
      rename(original_index = Layer) %>%
      left_join(gene_mapping, by = "original_index") %>%
      select(-original_index)
  }
  ...
  if(is_gene_analysis) {
    # Add gene/subtype information to enrichment stats
  ...
  for(mask_name in names(named_density_masks)) {
    f1_col <- paste0(mask_name, "_F1")
    if(!(f1_col %in% names(comp_stats))) {
      comp_stats[[f1_col]] <- NA
    }
  }
  
  # Initialize new columns for enrichment statistics
  enrichment_cols <- setdiff(names(enrichment_stats), 
                             if(is_gene_analysis) c("subtype", "gene") else "Layer")
  for(col in enrichment_cols) {
    if(!(col %in% names(comp_stats))) {
      comp_stats[[col]] <- NA
    }
  }
  ...
  # Update values based on whether it's gene analysis or layer analysis
  if(is_gene_analysis) {
    # For gene analysis, match by both subtype and gene
    comp_stats <- comp_stats %>%
      left_join(f1_scores, by = c("subtype", "gene")) %>%
      left_join(enrichment_stats, by = c("subtype", "gene"))
  } else {
    # For layer analysis, match directly by Layer
    comp_stats <- comp_stats %>%
      left_join(f1_scores, by = "Layer") %>%
      left_join(enrichment_stats, by = "Layer")
  }
  
  # Save updated comprehensive statistics
  write.csv(comp_stats, comp_stats_path, row.names = FALSE)
  
  # Create and save igor version (with NAs replaced by -1000)
  comp_stats_igor <- comp_stats %>%
    mutate(across(everything(), ~replace(., is.na(.), -1000)))
  
  write.csv(comp_stats_igor, 
            paste0(substr(comp_stats_path, 1, nchar(comp_stats_path)-4), "_igor.csv"), 
            row.names = FALSE)
